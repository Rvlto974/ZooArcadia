...
async function register(req, res) {
  ...
  try {
    ...
    // L'inscription publique crée toujours un employé : seul un flux administrateur peut créer un admin.
    const password_hash = await bcrypt.hash(password, 12);
    const user = await userModel.createUser({ email, password_hash, role: 'employee' });
    return res.status(201).json({ message: 'Compte créé avec succès.', user: publicUser(user) });
  } catch (error) {
    // Une contrainte UNIQUE peut gagner une course entre deux inscriptions simultanées.
